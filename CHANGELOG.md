...
- Every cache disk operation runs on a store thread, never on a serving worker (#286). `cache.from_config` with a disk root starts `hedge.cache.store_worker`, a supervisor-owned thread that runs each `disk.Request` a worker submits. The submitting call parks, and the completion reaches the submitter through its own `io_runtime.wake`: `wake.Queue` gains a locked inbox (`wake.post`, `wake.receive`, `wake.posted`, `attach_runtime`, `detach_runtime`) whose settle runs on the owner's thread. `serve.start` attaches the worker's runtime, `route_wakes` and the stop quiesce loop receive the inbox, and the runtime is detached before the listener stops. The queue is bounded by admission: `disk.Executor` gains `admit` and `leave`, and `disk.admit` and `disk.leave` bracket one body's requests (64 bodies, plus a reserve for removals and the purge). A refused hit is served from the origin and counted as a miss, and a refused disk recording is skipped with its response intact. A disk hit reads its first 16 KiB into a per-worker read slot before the response commits (16 slots per worker), and the rest streams from that slot with one chunk read ahead. `cache.close` takes the stop timeout and drains the queue, and `cache.stalled` and `composition.cache_stalled` name what a timed-out stop left, which the binary prints. `store.attach_disk` takes the executor, `call.detach_finalizer` removes a finalizer its owner attached, and `serve.attach_plane_audit` lets the debug audit ask a plane whether it holds work while not due.
- The cache store is safe to share between workers (#285). It is split into shards by the FNV-1a hash of the cache key (`store.make` and `cache.from_config` take the shard count, and the composition passes one, which is exactly the unsharded store). Each shard owns its slots, its share of the body arena and of the memory, disk and entry budgets, and a `std.sync` mutex. The lock is held only across index and arena work: `store.acquire` and `store.release` bracket work on a shard, and functions taking a `*Shard` require it held. A reader pins an entry (`store.pin`, `store.unpin`) before it streams. Eviction, replacement or invalidation of a pinned entry unlinks it from the index at once, and its bytes are freed at its last unpin. Debug builds fill freed arena bytes with `store.POISON`. A hit is answered from a `store.Snapshot` copied into the call's arena under the lock, so a refresh on another worker cannot change fields a response points into. Every disk operation is now a request (`hedge.cache.disk`): a read, a write, a file removal or the startup purge is submitted and completes later, and no request is made with a shard lock held. The inline executor ran each request on the thread that submitted it until the store worker (#286) replaced it. A queued read answers `body.pending` and wakes the call. A reclaimed disk entry keeps its slot and disk charge until its file is gone. Recordings belong to the worker that makes them: `cache.Recorders` (`make_recorders`, `attach_waker`, `drive`, `settling`, `close_recorders`) holds a worker's recordings and, with a disk root, 32 KiB of staging per recording for disk writes. `cache.Bindings` and `cache.Binding` name the worker's `Recorders`. A recording that ends with a write in flight settles through the composition's plane before stop quiesces.
