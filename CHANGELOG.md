...
- A laurel application's `handler_deadline_ns` is enforced (#171). hedge reads laurel's narrowed deadline after binding a request and times out the exchange scope once a suspended handler is still waiting past it.
...
- The TLS read preemption from 0.5.3 (#196) can drop bytes on Windows until hedge adopts mach-std 5.3.0 (#149). Before 5.3.0, mach-std's Windows backend discards a socket read whose completion races its cancel, and that includes the mach-std 4.2.0 hedge pins. A record that arrives just as a response is queued then never reaches mach-tls, and the connection fails. Linux and darwin are unaffected by that discard: their backends are readiness-based and read only inside `poll`, which hands every finished read out before hedge can cancel it. mach-std also zeroed a raced read's bytes on those platforms in a narrower window, between a cancel being claimed and `poll` finishing the operation. hedge cancels a read and polls its runtime on the same thread, and std starts no thread of its own, so that window does not open for hedge. mach-std 5.3.0 delivers every raced read as cancelled with its bytes, and mach-tls has always taken those bytes, so the preemption is sound once hedge is on 5.3.0. The preemption itself is temporary: mach-tls 0.8.0 (briar-systems/mach-tls#103) lets a stream run one read and one write at once, and the preemption goes when hedge adopts it. hedge never cancels a TLS write this way. A write cancelled by `write_ms` fails the connection, and before mach-tls 0.8.0 that failed stream also reports zero application bytes even when the peer received the record.
