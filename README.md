...
implemented. A `transport = "quic"` listener binds, becomes ready, and carries
a QUIC connection through address validation, admission, and connection
assembly, but the TLS handshake inside QUIC does not yet complete, so HTTP/3
is not served (#32). The current Let's Encrypt chain uses certificate
algorithms mach-tls cannot verify (#38), and a listener's credential
generation cannot yet be replaced (#37).
